...
int find_entry_internal(const char* path, DirEntrySlot* slot, const char** remains) {
    ...
    *remains = next_level;
    while (true) {
         ...
         */
        // ================== Your code here =================
        sector_t sec = cluster_first_sector(clus); // 获取当前簇的第一个扇区
        size_t nsec = meta.sec_per_clus; // 当前簇的扇区数 
        state = find_entry_in_sectors(*remains, len, sec, nsec, slot); // 在当前簇的扇区中查找目录项
        if(state < 0) { // 查找出错
            return state;
        }
...
int read_from_cluster_at_offset(cluster_t clus, off_t offset, char* data, size_t size) {
    ...
    while(pos < size) { // 还没有读取完毕
        ...
        // ================== Your code here =================
        memcpy(data + pos, sector_buffer + sec_off, min(size - pos, meta.sector_size - sec_off)); // 从扇区的 sec_off 开始复制到 data 中
        pos += min(size - pos, meta.sector_size - sec_off); // 更新 pos
        // ===================================================
        sec_off = 0;
        sec ++ ;
    }
...
int dir_entry_create(DirEntrySlot slot, const char *shortname, 
    ...
    memcpy(dir->DIR_Name, shortname, 11); // 将短文件名复制到目录项中
    dir->DIR_Attr = attr; // 设置目录项属性
    dir->DIR_FstClusHI = 0; // 在 FAT16 中，DIR_FstClusHI 永远为 0
    dir->DIR_FstClusLO = first_clus; // 设置文件的首个簇号
...
int write_fat_entry(cluster_t clus, cluster_t data) {
    ...
    for(size_t i = 0; i < meta.fats; i++) {
         ...
         */
        // ================== Your code here =================
        sector_t fat_sec_i = meta.fat_sec + i * meta.sec_per_fat;   // 计算第i个FAT表所在扇区
        sector_t clus_in_fat = fat_sec_i + clus_sec;    // 计算clus对应的FAT表项所在扇区
        int ret = sector_read(clus_in_fat, sector_buffer);  // 使用 sector_read 读取扇区
        if(ret < 0){
            return ret;
        }
        memcpy(sector_buffer+sec_off, &data, sizeof(cluster_t));     // 使用 memcpy 将 data 写入 buffer
        ret = sector_write(clus_in_fat, sector_buffer);     // 使用 sector_write 写回扇区
        if(ret < 0){
            return ret;
        }
        // ===================================================
...
int alloc_one_cluster(cluster_t* clus) {
    ...
    // ================== Your code here =================
    cluster_t first_free = CLUSTER_END; // 用于保存找到的第一个空闲簇
    for(cluster_t i = 2; i < meta.clusters + 2; i++) { // 从2开始，0和1是保留的
        if(read_fat_entry(i) == CLUSTER_FREE) { // 如果簇是空闲的
            first_free = i; // 找到第一个空闲簇
            break;
        }
    }
    if(first_free == CLUSTER_END) { // 没有找到空闲簇
        return -ENOSPC;
    }
    int ret = write_fat_entry(first_free, CLUSTER_END); // 修改FAT表项，将其指向CLUSTER_END
    if(ret < 0) {
        return ret;
    }
    ret = cluster_clear(first_free); // 清零该簇
    if(ret < 0) {
        return ret;
    }
    *clus = first_free; // 将分配的簇号返回
    // 注意：这里不需要释放 clusters，因为我们没有使用 malloc 分配内存。
    // ===================================================
    return 0;    // TODO 请删除这一行或者修改为正确的返回值
}
...
int alloc_clusters(size_t n, cluster_t* first_clus) {
    ...
    for(cluster_t i = 2; i < meta.clusters + 2; i++) { // 从2开始，0和1是保留的
        if(read_fat_entry(i) == CLUSTER_FREE) { // 如果簇是空闲的
            *(clusters + allocated) = i;    // 找到空闲簇
            allocated++;    // 更新已找到的空闲簇个数
            if(allocated == n)
                break;
        }
    }
    if(allocated < n){  // 找不到n个簇
        free(clusters);
        return -ENOSPC;
    }
    for(size_t i = 0; i < n; i++) { // 修改clusters中前N个簇
        if(i < n-1) {   // 如果是前n-1个簇，需要将每个簇与下一个簇连接在一起
            int ret = write_fat_entry(clusters[i], clusters[i+1]);
            if(ret < 0) {
                return ret;
            }
            ret = cluster_clear(clusters[i]);   // 清零每一个新分配的簇
            if(ret < 0) {
                return ret;
            }
        } else {    // 如果是最后一个簇，需要将其设置为CLUSTER_END
            int ret = write_fat_entry(clusters[i], CLUSTER_END);
            if(ret < 0) {
                return ret;
            }
            ret = cluster_clear(clusters[i]);   // 清零每一个新分配的簇
...
int fat16_mkdir(const char *path, mode_t mode) {
    ...
    // ================== Your code here =================
    ret = find_empty_slot(path, &slot, &filename);  // 找一个空的目录项
    if(ret < 0) {
        return ret;
    }
    ret = alloc_one_cluster(&dir_clus); // 分配一个空闲簇
    if(ret < 0) {
        return ret;
    }
    char shortname[11];
    ...
    ret = dir_entry_create(slot, shortname, ATTR_DIRECTORY, dir_clus, 0); // 创建目录项
    if(ret < 0) {
        free_clusters(dir_clus); // 如果创建目录项失败，释放分配的簇
        return ret;
    }
    ret = cluster_clear(dir_clus); // 清零新分配的簇
    if(ret < 0) {
        free_clusters(dir_clus); // 如果清零失败，释放分配的簇
        return ret;
    }
    // ===================================================
...
int fat16_unlink(const char *path) {
     ...
     */
    // ================== Your code here =================
    int ret = find_entry(path, &slot); // 寻找文件对应的目录项
    if(ret < 0){
        return ret;
    }
    if(attr_is_directory(dir->DIR_Attr) == 0){ // 确认目录项是个文件
        ret = free_clusters(dir->DIR_FstClusLO);
        if(ret < 0) {
            return ret;
        }
        // 修改目录项为删除
        memset(dir, 0, sizeof(DIR_ENTRY)); // 将目录项清零
...
int fat16_rmdir(const char *path) {
    ...
    DirEntrySlot slot = {{}, 0, 0};
    DIR_ENTRY* dir = &(slot.dir);
    int ret = find_entry(path, &slot); // 寻找目录对应的目录项
    if(ret < 0) {
        return ret;
    }
    if(!attr_is_directory(dir->DIR_Attr)) { // 确认目录项是个目录
        return -ENOTDIR;
    }
    // if(dir->DIR_FstClusLO == 0) { // 根目录没有簇号，无法删除
    //     return -EBUSY;
    // }
    if(path_is_root(path)) {
        return -EBUSY; // 如果目录项是根目录，无法删除
    } 
    cluster_t clus = dir->DIR_FstClusLO; // 获取目录的簇号
    // 检查目录是否为空
    char sector_buffer[MAX_LOGICAL_SECTOR_SIZE];
    sector_t first_sec = cluster_first_sector(clus);
    size_t nsec = meta.sec_per_clus; // 获取目录的扇区数
    for(size_t i = 0; i < nsec; i++) {
        sector_t sec = first_sec + i;
        ret = sector_read(sec, sector_buffer); // 读取目录所在的扇区
        if(ret < 0) {
            return ret;
        }
        // 检查每个目录项是否为空
        for(size_t j = 0; j < meta.sector_size ; j += DIR_ENTRY_SIZE) {
            ...
            if(entry->DIR_Name[0] != 0 && entry->DIR_Name[0] != 0xE5) { // 如果目录项不为空
                const char DOT_NAME[] =    ".          ";
                const char DOTDOT_NAME[] = "..         ";
                if(memcmp(&(entry->DIR_Name), DOT_NAME, 11) == 0 || memcmp(&(entry->DIR_Name), DOTDOT_NAME, 11) == 0) {
                    continue; // 跳过 . 和 .. 目录项
                }
                printf("find non-empty entry: %s\n", entry->DIR_Name);
                return -ENOTEMPTY; // 目录不为空，无法删除
            }
        }
    }
    // 目录为空，可以删除
    ret = free_clusters(clus); // 释放目录占用的簇
    if(ret < 0) {
        return ret;
    }
    // 修改目录项为删除
