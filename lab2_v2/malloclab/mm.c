...
void *mm_malloc(size_t size)
{
    ...
    if ((bp = find_fit_first(newsize)) != NULL)
    {
        ...
        place(bp, newsize);
        // printf("\n******** AFTER place():");     // debug
        mm_check(__FUNCTION__, bp);             // debug
...
static void* find_fit_best(size_t asize) {
    ...
    */
    void *bp = free_listp;
    void *best_bp = NULL;
    size_t best_size = 0;  // 初始化为0，在第一轮循环中更新
...
static void place(void *bp, size_t asize)
{
    ...
    */
    ...
    if (free_size < MIN_BLK_SIZE) {
        ...
        // 更新 next block 的 prev_alloc 域为 1
        void *next_bp = NEXT_BLKP(bp);
            size_t next_size = GET_SIZE(HDRP(next_bp));
        ...
        PUT(HDRP(free_bp), PACK(free_size, 1, 0));   // 头部
        PUT(FTRP(free_bp), PACK(free_size, 1, 0));   // 尾部

        add_to_free_list(free_bp);
        user_malloc_size += asize - WSIZE; // user space need minus header
    }
   mm_check(__FUNCTION__, bp);
}
...
void mm_check(const char *function, char* bp)
{
    #ifdef DEBUG
    printf("\n---cur func: %s :\n", function);
    printf("addr_start: 0x%lx, addr_end: 0x%lx, size_head: %lu, size_foot: %lu, PRED=0x%lx, SUCC=0x%lx, NEXT_head=0x%lx \n", (size_t)bp - WSIZE,
        (size_t)FTRP(bp), GET_SIZE(HDRP(bp)), GET_SIZE(FTRP(bp)), GET_PRED(bp), GET_SUCC(bp), GET(HDRP(NEXT_BLKP(bp))));
    fflush(stdout);
    #endif
    // head_state: %lu, foot_state: %lu,
    // (GET(HDRP(bp)) & 0x3), (GET(FTRP(bp)) & 0x3),
}
