...
// TODO:修改为你的系统调用号
#define SYS_trace  [] 
#define MAX_SYSCALLS 400  // 最大系统调用号数量
...
int main(int argc, char *argv[]) {
    ...
    // TODO:遍历syscall_names来匹配系统调用号(regexec(&regex, syscall_names[nr], 0, NULL, 0)函数返回0表示匹配成功)

    int ret;
    for(int nr = 0; nr < MAX_SYSCALLS; nr++){
        ret = regexec(&regex, syscall_names[nr], 0, NULL, 0);
        if(ret == 0){
            //匹配成功，找到了系统调用号
            break;
        }
