...
static int hello_getattr(const char *path, struct stat *stbuf, struct fuse_file_info *fi) {
	...
	// ================== Your code here =================
	else if (strcmp(path+1, "PB23111688") == 0) { // 替换 your_student_id 为你的学号
		stbuf->st_mode = S_IFREG | 0444; 	// 普通文件，权限是 0444 （八进制）
		stbuf->st_size = strlen("PB23111688\n"); // 替换为你的学号内容
	}
	// ===================================================
	else {
			...
			off_t offset, struct fuse_file_info *fi, enum fuse_readdir_flags flags)
{
	...
	// ================== Your code here =================
	filler(buf, "PB23111688", NULL, 0, 0); // 替换为你的学号
		      ...
		      struct fuse_file_info *fi)
{
	...
	// ================== Your code here =================
		...
		file_data = "PB23111688\n";	// 文件内容为你的学号
		file_size = strlen(file_data);	// 文件大小
	}
